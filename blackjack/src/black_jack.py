...
def monte_carlo_es(episodes):
    ...
    for episode in tqdm(range(episodes)):
        ...
        initial_state = [
            bool(np.random.choice([0, 1])),  # usable_ace flag
            np.random.choice(range(12, 22)),  # player_sum between 12 and 21
            np.random.choice(range(1, 11))  # dealer's showing card (1 to 10)
        ]
