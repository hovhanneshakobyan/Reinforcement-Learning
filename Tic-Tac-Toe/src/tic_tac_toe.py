...
def train(epochs: int, print_every_n: int = 500):
    ...
    for epoch in range(1, epochs + 1):
        # For every epoch
        winner = judge.play(all_states)  # get the winner
        ...
        if epoch % print_every_n == 0:
            print(f'epoch {epoch}, win rate player 1: {player1_win_rate}, win rate player 2: {player2_win_rate}')
...
def play():
    # region Summary
    """
    Play against RL player. The game is a 0-sum game.
